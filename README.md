...
| Skill        | Owns                                                                                                         |
|--------------|--------------------------------------------------------------------------------------------------------------|
| [`router`](./router/SKILL.md)     | Detects intent, loads the right sibling(s), restates suite-wide guardrails, brokers patterns. Always loaded. |
| [`design`](./design/SKILL.md)     | Information architecture, navigation, landing/hub pages, component selection.                                |
| [`write`](./write/SKILL.md)       | Voice, frontmatter, file conventions, code-example standards. Vendors upstream Mintlify writing standards.   |
| [`create`](./create/SKILL.md)     | Bootstrapping a new site: `mint init`, baseline `docs.json`, CI starter, redirects.                          |
| [`maintain`](./maintain/SKILL.md) | Ongoing health: `mint broken-links` / `validate` / `a11y` / `update`, lychee CI, staleness PRs.              |

A sixth, opt-in directory — [`patterns/`](./patterns) — holds site-type templates (API docs, SaaS, learning site, resource library, internal wiki, multi-product). Patterns are *menu items*, never defaults.

## Loading model

The router is small (~12KB, well under any companion skill) and **always loaded** when the suite is active. Companion skills load on demand based on the user's intent. A typical session loads at most two companion skills; total active context stays under ~30KB.

| Intent                                              | Loads                       |
|-----------------------------------------------------|-----------------------------|
| "Restructure my nav" / "pick a component"           | router + `design`           |
| "Write or edit prose"                               | router + `write`            |
| "Stand up a fresh repo"                             | router + `create` + `design`|
| "Link rot / staleness scan"                         | router + `maintain`         |
| "Add a card linking to this URL"                    | router + `write`            |
...
**v1.0.0 — initial release** (in progress). Versioning is semver.

## License

[MIT](./LICENSE) © TumbleweedLabs.

The `write` skill vendors writing-standards content from the upstream [`mintlify/docs` skill](https://github.com/mintlify/docs/blob/main/skill.md) with credit and a link back. The `design` skill is descended from the standalone [`TumbleweedLabs/mintlify-design-skill`](https://github.com/TumbleweedLabs/mintlify-design-skill), which is archived and superseded by this suite.
